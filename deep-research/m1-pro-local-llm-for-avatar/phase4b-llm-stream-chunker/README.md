...
### 次回検証計画 — verify-persona.py 多様化版 (A2 + D5)

#### 起点

- **副次発見** (Phase 5 [PR #8](https://github.com/linnefromice/ai-research-lab/pull/8) サブタスク 3-7 実装ログ #5):
  persona の hard-code フレーズ「んー、あんまり詳しくないかも」 が LLM に定型語尾として
  過学習。verify-persona.py の 1 種類 prompt × 5 trials では見えなかった現象が、Phase 5 実機の
  幅広い質問パターン (挨拶、子育て相談、確認質問) で初めて表面化
- **Phase 4b 反省**: verify-persona.py が prompt 1 種類しか試さなかったため、persona V1 の
  副作用 (副次発見 5) を見逃した
- **A2 (persona V2 定量検証)** + **D5 (verify-persona.py 多様化)** は同じ tool で達成できる:
  「prompt × persona × trial」 matrix の検証 script
...
| 軸 | 値 |
|---|---|
| persona | V1 (旧 NEW_SP、hard-code フレーズあり) / V2 (Phase 5 で投入、hard-code フレーズ削除) |
| prompt | 5 種類 (下記) |
| trial | 各 5 回 |

prompt 5 種類 (副次発見の表面化条件をカバーする):

1. 「好きなアニメ教えて」 — sanctioned 領域、confabulation テスト (verify-persona.py から継承)
2. 「おはよう」 — 挨拶、persona 一貫性 (Phase 5 で「あんまり詳しくない」 が誤発火した文脈)
3. 「今日の天気どう？」 — 一般、知らない領域 (hedge phrase の正当な使用)
4. 「子供と何して遊んだらいい？」 — 相談、長文化リスク (副次発見 3 の markdown bullet list 出現条件)
5. 「ナオちゃん元気？」 — キャラ呼びかけ (一人称 / 二人称使い分け)

出力指標 (各組み合わせで集計):

| 指標 | 連動する副次発見 |
|---|---|
| sanctioned 外作品の出現率 (#1 で測定) | persona 拡張 A/B 検証の継続 |
| 「あんまり詳しくないかも」 出現率 (全 prompt) | 副次発見 5 (hard-code 過学習) |
| 3 文超過率 (全 prompt) | 副次発見 3 (3 文制約破り) |
| markdown bullet list 出現率 (全 prompt) | 副次発見 3 |
| 1 文目 length 平均 (全 prompt) | 応答 length 制約の遵守度 |

#### 実装方針

- ファイル: `phase4b-llm-stream-chunker/verify-persona-matrix.py` (verify-persona.py の上位互換、stdlib のみ)
- 入力: persona V1 / V2 + prompt 5 種類 を script 内に hard-code (将来必要なら YAML 化)
- 出力:
  - JSON ログ (各 trial の prompt / persona / response / 計算済み指標)
  - markdown サマリ table (lab repo に貼り付けられる形式)
- 試行コスト: 50 trials = LM Studio + temp=0.7 で 数分程度

#### 工数 + 着手タイミング

- script 実装: 30-45 分 (verify-persona.py の matrix 化)
- 実行: 数分
- 結果分析 + lab repo に記録: 30 分
- **合計: 1-1.5 時間**

着手タイミング候補:
- (a) **Phase 6 着手前** — D4 実装 (δ + ε) のベースライン測定として実施、persona V2 効果も同時証明
- (b) **Phase 6 D4 実装直後** — V3 (D4 ε で投入) と V2 の比較として再利用
- (c) **D4 実装中の continuous validation** — 修正のたびに matrix 再実行で regression 防止

→ **既定推奨: (a)**。Phase 6 のベースライン測定 + persona V2 の証明 + matrix script の温度チェック を 1 タスクで達成できる。Phase 6 サブタスク 0 として組み込みたい。
