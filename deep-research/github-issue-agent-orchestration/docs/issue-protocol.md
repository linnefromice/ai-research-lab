...
`TARGET_REPO` への issue オーケストレーション用ラベル 16 種の作成は
`scripts/setup-issue-labels.sh` で行う（`gh label create --force` で冪等。何度
実行しても同じ状態に収束する）。ラベル定義（名前・色・説明）の正本はこの
スクリプトと本ファイル §2。issue テンプレ等それ以外の GitHub 側設定
（リポ作成・テンプレ配置）は本プロジェクトの範囲外でユーザーが行う。
