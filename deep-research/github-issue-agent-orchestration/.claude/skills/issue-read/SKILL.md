...
1. `.env` を読む: `set -a; . ./.env; set +a`（`TARGET_REPO` を得る）。
2. 取得:
   ```bash
   gh issue view <N> --repo "$TARGET_REPO" \
     --json number,title,body,labels,state,comments
   ```
3. ラベルから `type:` / `status:` / `agent:` / `priority:` を分離して抽出する。
4. 本文冒頭の `親: #<N>` / `由来: #<N>` 行を抽出する。
5. **規約チェック**: `status:` prefix のラベルがちょうど 1 個か検査する。0 個 / 2 個
   以上なら規約違反として警告する（[issue-protocol.md](../../../docs/issue-protocol.md) §4）。
6. 次を構造化サマリとして返す: 番号 / title / type / status / agent / priority /
