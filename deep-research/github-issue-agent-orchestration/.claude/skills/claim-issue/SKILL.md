...
2. **再確認**: `gh issue view <N> --repo "$TARGET_REPO" --json labels,state` で
   現在も `status:ready` かつ open か検査する。
   - 既に `status:in-progress` 等に変わっている / closed → **claim 失敗**。
     呼び出し元に「他者が claim 済み」と返し、この issue はスキップさせる。
3. `ready` のままなら `issue-transition` で `ready`→`in-progress` へ atomic 遷移する。
4. claim 成功を返す（issue 番号 + 現 status）。
